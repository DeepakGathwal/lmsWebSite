...
export async  function POST(req){
  ...
  const { profile, provider } = await req.json()
  const {name, email, picture, avatar_url} = await profile
   const query = `Select id from jtc_ecommers_users WHERE email = '${email}' && deleted_by = '0'`
  const runQuery =  await executeQuery(query)
  if(runQuery.length > 0){
      const insertUser =  `Update  jtc_ecommers_users SET last_login = current_timestamp() WHERE email = "${email}" && provider = "${provider}"`
      const runInsertQuery =  await executeQuery(insertUser)
      if(runInsertQuery.affectedRows > 0)
   return  NextResponse.json({success : true}, {data:"Login Successfully"}, {status : 200})
  }else {
  const otp = await Math.floor(1000 + Math.random() * 9000);
    const insertUser =  `Insert into jtc_ecommers_users SET email = "${email}", name = "${name}", provider = "${provider}", image = "${picture ? picture : avatar_url}", otp = '${otp}'`
   
   
   
    const runInsertQuery =  await executeQuery(insertUser)
    if(runInsertQuery.affectedRows > 0)
         return NextResponse.json({message : 'verify the otp'}, { success : true}, {status : 200})    
  }
}
...
export async  function PUT(req){
    ...
    const runQuery =  await executeQuery(query)
    if(runQuery.length > 0){
     return NextResponse.json({message : 'Account Already Exists'}, { success : false}, {status : 206})
  }else {
      const otp = await Math.floor(1000 + Math.random() * 9000);
    const newPassword = await bcryptjs.hashSync(`${password}`, 8)
    const insertUser =  `Insert into jtc_ecommers_users SET email = "${email}", name = "${name}", provider = "Self", password = '${newPassword}', otp = '${otp}'`
...
<![endif]-->
 ...
 await sendEmail(option)
   ...
   return NextResponse.json({message : 'verify the otp'}, { success : true}, {status : 200})    }
   else NextResponse.json({message : 'DataBase Issue'}, { success : true}, {status : 206}) 
    }
}
...
export async  function PATCH(req){
    ...
    const query = `Select id from jtc_ecommers_users WHERE email = '${email}' && deleted_by = '0' && otp = '${otp}'`
    const runQuery =  await executeQuery(query)
    if(runQuery.length > 0) return NextResponse.json({message : 'Account Verify'}, { success : true}, {status : 200})
  else return NextResponse.json({message : 'Account Not Found'}, { success : false}, {status : 206})    
